...
if __name__ == "__main__":
    ...
    while not sent:
        ...
        if 'TemporaryChannelFailure' in pe:
            print(f'TCF on the {pe[-2:]} hop')
            hop_error_index = int(pe[-1:]) - 1
            source_pubkey = route['route']['hops'][hop_error_index-1]
            dest_pubkey = route['route']['hops'][hop_error_index]
            print(source_pubkey, dest_pubkey)
